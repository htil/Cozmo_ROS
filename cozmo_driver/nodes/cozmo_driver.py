...
class CozmoRos(object):
    ...
    def _set_backpack_led(self, msg):
        ...
        temp = [int(x * 255) for x in [msg.r, msg.g, msg.b]]
        color = [temp[0], temp[1], temp[2]]
        # create lights object with duration
        if(int(msg.a) == 1):
            light = cozmo.lights.Light(on_color=cozmo.lights.red ,off_color=cozmo.lights.blue, on_period_ms=500, off_period_ms=500)
    ...
    def _say_callback(self, msg):    
        if(("tide" in msg.data.lower()) or ("roll" in msg.data.lower())):
            msg.data = "I can't say that."
        print(msg.data)
        """
